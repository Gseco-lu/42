...
/*   Updated: 2021/11/23 17:12:07 by gseco-lu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "get_next_line_bonus.h"

char	*glue(char **stor, char **buffer, int len)
{
	char	*ret;
	char	*temp;
	int		i;
...
char	*saving(char **stor, char **buffer, int r)
{
	char	*line;
	char	*temp;
	...
	if (temp)
		line = glue(stor, buffer, (temp - *buffer) + 1);
	else
	{
...
char	*get_next_line(int fd)
{
	...
	if (!buffer)
	{
		...
		while (line == NULL && r > 0)
		{
			...
			line = saving(&storage[fd], &buffer, r);
		}
		free(buffer);
	}
	else
		line = final(&storage[fd], (buffer - storage[fd]) + 1);
	return (line);
}
